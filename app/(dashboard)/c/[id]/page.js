...
export default function PostPage() {
 ...
 const {refresh, setRefresh}= useRefresh();
  ...
  const fetchPostPromptsMessage =async()=>{
	setIsLoading(true)
const response = await apiCaller(Post.getPrompts(params.id),'GET',null,'json',true)
if (response){
  ...
  const fetchPostData = async () => {
    try {
      const data = await apiCaller(Post.getPostById(params.id), "GET", null, "json", true);
setIsLoading(true)
      if (data) {
        ...
        const mappedPost = {
          ...
          sections: {
            hook: {
              id: "hook",
              type: "hook",
              content: data.ai_generated_content?.hook || "Are you ready to start?",
              style: "insightful",
              isEditing: false,
            },
            body: {
              id: "body",
              type: "body",
              content: data.ai_generated_content?.body || "Content goes here...",
              style: "insightful",
              isEditing: false,
            },
            call_to_action: {
              id: "call_to_action",
              type: "CTA",
              content: data.ai_generated_content?.call_to_action || "Call to action goes here...",
              style: "insightful",
              isEditing: false,
            },
          },
...
const handleRegenerate = async (sectionId, selectedStyle) => {
	...
	try {
	  ...
	  const aiResponse = await apiCaller(Post.regeneratePostSection(post.id), "POST", payload, "json", true);
...
setIsLoading(true)
	try {
	  ...
	  const response = await apiCaller(Post.chatPrompt(params.id), "POST", { prompt:message }, "json", true);
  ...
  const handleTitleChange = async(title)=>{
	console.log(title);
	await apiCaller(Post.updatePost(params.id),'PUT',{title:title},'json',true)
  }
  
  

  if (!post) return <div className="flex items-center justify-center h-screen">Loading post...</div>;
  ...
  return (
    <div className="flex h-[calc(100vh-0rem)] gap-6 p-6 bg-gray-100">
      <div className="w-[350px] overflow-hidden rounded-lg border bg-background">
        <ChatSidebar isLoading = {isLoading} onSendMessage={handleSendMessage} messages={promptsMessage} disabled={isLoading} />
      </div>
